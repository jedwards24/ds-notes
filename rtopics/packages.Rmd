...
[naniar](https://github.com/njtierney/naniar). Missing data functions. Tidy-based. Has a good collection of functions to replace strings with NAs (dplyr has `na_if()` but these are more usuable). Related [visdat](https://cran.r-project.org/web/packages/visdat/vignettes/using_visdat.html) has a few possibly useful plotting functions for missing values.
...
[Rando](https://github.com/MyKo101/rando) Context aware reproducible random numbers. Stronger checks than base.

[fs](https://fs.r-lib.org/) File system operations. More consistent naming and return values than base with better checks.  
...
From HW keynote RStudio Conf 2021

Lifecycle: 

* Stable, superceded are “in-warranty”, 
* Experiential, Deprecated are “out-of-warranty”.

“Off-label” use of functions (using for unintended side-effects) voids the warranty (may change in future).

To use a package outside of the update cycle use renv, RStudio package manager, or Microsoft’s mran.
