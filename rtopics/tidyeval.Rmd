...
There is an online bookdown [Tidy Evaluation](https://tidyeval.tidyverse.org/). This is still a work-in-progress as of writing (with incomplete parts). The [Programming dplyr vignette](https://dplyr.tidyverse.org/articles/programming.html) also covers similar ground and may be a better starting point. The [metaprogramming chapter in Advanced R](https://adv-r.hadley.nz/metaprogramming.html) has more on the general area.
...
dt <- tibble::tibble(
  ...
  return = c("quosure", "name","quosure", "name"),
  type = c("quoting", "quoting", "evaluating", "evaluating") 
) 
edwards::my_kable(dt)
```
...
Replacing `expr(mpg)` with `quo(mpg)` or `quo("mpg")` does error. since `sym()` cannot work with a quosure. Note that I have tried to use this expr/sym combo to solve the ggplot problem below but it is not a solution.
...
Other verbs like `filter()` work like ggplot, not `select()` and behaviour can be suprising at first. 
...
What is happening is that `!!enquo(x)` (or equivalently `{{`) is evaluating to the string `"vs"` and `"vs" == 1` is false. Like `ggplot()`, `filter()` doesn't accept variable names as strings due, I assume, to ambiguity. Because `sym()` creates a symbol this isn't evaluating to a string.
...
Before looking at alternatives there is still the question of what is the appropriate way for this sort of function to work. Should I commit to only accepting unquoted inputs to avoid any ambiguity? Also, at some point I will need to think about how different methods of handling tidyselect inputs will be effected by grouping.
...
**Notes from [lazyeval vignette](https://cran.r-project.org/web/packages/lazyeval/vignettes/lazyeval.html)**
...
Formulas are tool that captures (1) an unevaluated expression and, (2) the context (environment) in which the expression was created. The `~` character says: “I want to capture the meaning of this code, without evaluating it right away” (it is a sort of “quoting” operator). Technically, a formula is a “language” object (i.e. an unevaluated expression) with a class of “formula” and an attribute that stores the environment. lazyeval gives some functions that are useful for evaluating formulas.

(I note) Formulas can be constructed as strings then converted to a formula using `as.formula()`. 