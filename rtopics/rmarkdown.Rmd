...
+ [Rmd cheatsheet 2](https://www.rstudio.com/wp-content/uploads/2016/03/rmarkdown-cheatsheet-2.0.pdf) - this one is
newer but the other is less dense.
...
[Another article](https://rmarkdown.rstudio.com/articles_verbatim2.html) gives a function (I think). Need to look in detail.
...
Here is some adapted code to add a sparkline column. Here I use data added a simple intger vector. The sparkline is created by `sparkline()` then converted to text for `formattable()`. The two lines after the `df` creation tell the `formattable` HTML widget that it contains sparklines.
...
Rhian set up a personal academic website for me on github. It is hosted at https://james-edwards.netlify.com/. It was created using blogdown, which is a package, based on R markdown, used to create websites. There is an [online book](https://bookdown.org/yihui/blogdown/) about its use. There is a [guide](http://www.mit.edu/~k2smith/post/getting-started/) to the Hugo academic website theme. Also see [Nemeth’s github](https://github.com/chris-nemeth/academic-kickstart) for his website.

## Structure in R

There are four top level folders:  public, content, static, themes.
...
If unsure how I did something in the past then the best place to look is my commits.
