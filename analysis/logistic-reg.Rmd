...
**Sampling Bias**. Faraday (2006 book) notes that is outcomes are sampled with bias, then this only leads to a difference in intercept so that the relative effect of covariates can still be found. This is not true for other link functions. This assumes that the sampling bias is the same across all covariates. 
...
The Casella paper linked earlier has more detail on Bayesian options. A Gibbs sampler is used to get uncertainty estimates of coefficients. There is a part illustrating how bootstrap estimates for lasso coefficients are not reliable. 
