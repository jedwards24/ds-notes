...
Comparing `read_csv()` with`fread()`:

+ `read_csv()` misclassifies a lot of column types (but gives warnings). It bases type only on the top part of the column whereas `fread()` samples from different parts of columns.
...
+ `read_csv()` read “-” as “\x96” while fread seemed correct. This can is fixed using argument `locale = readr::locale(encoding = "windows-1252")`. See [github issue](https://github.com/tidyverse/readr/issues/892) and 
`?readr::guess_encoding`.
+ Attributes:
    + `fread()` has ".internal.selfref".
    + `read_csv()` attaches “spec”.
+ Speed:
    + It is usually said that `fread()` is quicker.
    + Generally the two packages seem similar when tested up to 2.5Gb. This was quite wide data. `vroom()` was not any quicker.
    + `read_csv()` is actually faster with higher guess_max than lower when lower value causes errors.
